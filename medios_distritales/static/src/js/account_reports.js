odoo.define('medios_distritales.account_report', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
        'value_changed': function(ev) {
             ...
             self.report_options.contable_ids = ev.data.contable_ids;
             return self.reload().then(function () {
                 ...
                 self.$searchview_buttons.find('.account_contable_filter').click();
             });
         },
    },
    ...
    render_searchview_buttons: function() {
        ...
        // account filter
        if (this.report_options.contable_ids) {
            console.log('pasa por aquí')
            console.log(this.report_options.contable_ids)
            if (!this.M2MFilters) {
                var fields = {};
                if ('contable_ids' in this.report_options) {
                    fields['contable_ids'] = {
                        label: _t('Cuentas contables'),
                        modelName: 'account.account',
                        value: this.report_options.contable_ids.map(Number),
                    };
                }
                if (!_.isEmpty(fields)) {
                    this.M2MFilters = new M2MFilters(this, fields);
                    this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_contable_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_contable_m2m').append(this.M2MFilters.$el);
            }
        }
    },
