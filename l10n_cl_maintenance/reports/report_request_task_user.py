...
class ReportRequestTaskUser(models.Model):
    _name = "report.request.task.user"
    _description = "Tasks Analysis"
    _order = 'name desc, request_id'
    _auto = False
    ...
    stage_id = fields.Many2one('maintenance.request.task.stage', string='Stage', readonly=True)
    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE view %s as
            %s
            FROM maintenance_request_task t
            INNER JOIN maintenance_request p ON t.request_id = p.id 
            WHERE t.active = 'true'
                %s
        """ % (self._table, self._select(), self._group_by()))
