...
class RPT55(models.Model):
    _name = 'report.equipment.its.failures'
    _description = '2-55-KPI- Fallas ITS por equipos'
    _auto = False
    ...
    def get_query(self, dates=False, date=False, data=False):
        ...
        query = f"create or replace view report_equipment_its_failures as ({sql})"

        return query

    def init(self):
        tools.drop_view_if_exists(self._cr, 'report_equipment_its_failures')
    ...
    def action_view_kpi(self, code_rpt_type=False, show_wz=False):
        ...
        message = f"<b>Total kilometraje: {total_odometer}<b/></br>" \
                  f"<b>Total buses: {total_equipment}<b/></br>" \
                  f"<b>Fallas ITS: <span class='text-danger text-bf'>{kpi:.2f}<span/><b/>"
        if not show_wz:
    ...
    def get_act_window(self, data):
        ...
        view_tree = self.env.ref('report_maintenance_mblz2.report_equipment_its_failures_tree')
        view_pivot = self.env.ref('report_maintenance_mblz2.report_equipment_its_failures_pivot')
        view_dashboard = self.env.ref('report_maintenance_mblz2.report_equipment_its_failures_dashboard')

        return {
            'name': data['name'],
            'type': 'ir.actions.act_window',
            'res_model': 'report.equipment.its.failures',
            'view_mode': 'tree,dashboard',
            'views': [(view_tree.id, 'tree'), (view_pivot.id, 'pivot'), (view_dashboard.id, 'dashboard')],
